...
public abstract class AbstractPanel<T extends GameObject> extends AbstractModifiableContainer<T> implements Panel<T> {
    ...
    protected boolean onRemove(T obj, boolean recalculateBounds) {
        if (super.remove(obj)) {
            if (recalculateBounds) {
                recalculateBounds();
            }
